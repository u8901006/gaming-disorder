...
async function analyzePapers(apiKey, papersData) {
  ...
  const paperCount = papersData.new_count || papersData.papers?.length || 0;
  const papersText = JSON.stringify(papersData.papers || [], null, 2);

  const prompt = `以下是 ${dateStr} 從 PubMed 抓取的最新遊戲障礙（Gaming Disorder）研究文獻（共 ${paperCount} 篇新論文）。
  ...
  for (const model of modelsToTry) {
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        console.error(`[WARN] JSON extraction failed on attempt ${attempt + 1}`);
...
<title>Gaming Disorder Research &middot; 遊戲障礙研究文獻日報 &middot; ${dateDisplay}</title>
<meta name="description" content="${dateDisplay} 遊戲障礙研究文獻日報，由 AI 自動彙整 PubMed 最新論文"/>
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .link-text { flex: 1; }
  .link-title { font-size: 15px; font-weight: 700; color: var(--text); }
  ...
  @media (max-width: 600px) { .container { padding: 36px 18px 60px; } .summary-card, .news-card { padding: 20px 18px; } footer { flex-direction: column; gap: 6px; text-align: center; } .topic-name { width: 70px; font-size: 11px; } .links-section { gap: 8px; } .link-card { padding: 14px 18px; } }
</style>
</head>
<body>
<div class="container">
  <header>
    ...
    <div class="header-text">
      <h1>Gaming Disorder Research &middot; 遊戲障礙研究文獻日報</h1>
  ...
  <div class="links-section">
    <a href="https://www.leepsyclinic.com/" class="link-card" target="_blank">
      ...
      <span class="link-text">
        <span class="link-title">李政洋身心診所首頁</span>
    ...
    <a href="https://blog.leepsyclinic.com/" class="link-card" target="_blank">
      ...
      <span class="link-text">
        ...
        <span class="link-desc">李政洋身心診所部落格</span>
      </span>
      <span class="link-arrow">&rarr;</span>
    </a>
  ...
  <footer>
    <span>資料來源：PubMed &middot; 分析模型：${FALLBACK_MODELS[0]}</span>
    <span><a href="https://github.com/u8901006/gaming-disorder">GitHub</a></span>
  </footer>
</div>
</body>
</html>`;
...
function updateSummarizedCache(allPmids) {
  let existing = { pmids: [] };
  if (existsSync(SUMMARIZED_PATH)) {
    try {
      existing = JSON.parse(readFileSync(SUMMARIZED_PATH, "utf-8"));
    } catch {}
  }
  const set = new Set([...(existing.pmids || []), ...allPmids]);
  const arr = [...set];
  while (arr.length > 2000) arr.shift();
  writeFileSync(SUMMARIZED_PATH, JSON.stringify({ pmids: arr }, null, 2), "utf-8");
}
...
async function main() {
  ...
  const newCount = papersData.new_count ?? papersData.papers?.length ?? 0;

  if (!papersData.papers || !papersData.papers.length || newCount === 0) {
    console.error("[WARN] No new papers to summarize");
    const dateStr = papersData.date || new Date().toISOString().slice(0, 10);
    const emptyAnalysis = {
      date: dateStr,
      market_summary: "今日沒有新的遊戲障礙研究文獻。請明天再查看。",
      top_picks: [],
      all_papers: [],
      keywords: [],
      topic_distribution: {},
    ...
    const outPath = outputPath || resolve(ROOT, "docs", `gaming-${dateStr}.html`);
    writeFileSync(outPath, html, "utf-8");
    console.error(`[INFO] Empty report saved to ${outPath}`);
    return;
  }
  ...
  const dateStr = analysis.date || papersData.date || new Date().toISOString().slice(0, 10);
  const outPath = outputPath || resolve(ROOT, "docs", `gaming-${dateStr}.html`);
  ...
  if (papersData.all_pmids?.length) {
    updateSummarizedCache(papersData.all_pmids);
    console.error(`[INFO] Updated summarized cache with ${papersData.all_pmids.length} PMIDs`);
  }
}
